"""
وحدة قاعدة البيانات لوكيل التخطيط.

تلتزم هذه الوحدة بعزل التخزين وتشغيله داخل حدود الخدمة.
"""
...
from microservices.planning_agent.models import SQLModel
...
def create_db_engine(*, database_url: str, echo: bool, service_name: str) -> AsyncEngine:
    """إنشاء محرك قاعدة البيانات لوكيل التخطيط بصورة واضحة."""

    if not database_url:
        raise ValueError("DATABASE_URL غير مُعدّ لوكيل التخطيط.")
    logger.info("🔌 Database Ready: %s", service_name)
    return create_async_engine(database_url, echo=echo, future=True)


def create_session_factory(engine: AsyncEngine) -> async_sessionmaker[AsyncSession]:
    """إنشاء مصنع جلسات لقاعدة بيانات وكيل التخطيط."""
